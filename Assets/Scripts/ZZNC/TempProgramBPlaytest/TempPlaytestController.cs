    ...
    private void BuildLayout(bool useConfiguredInitialPieces = false)
    {
        ...
        foreach (var cell in _board.AllInsideCells())
        {
            ...
            foreach (var sr in obj.GetComponentsInChildren<SpriteRenderer>(true))
            {
                ...
                if (!isWall)
                    sr.color = new Color(0.827f, 0.224f, 0.118f, 0.306f);
            }
    ...
    private static readonly Dictionary<PieceType, (string Name, string Description)> PieceTooltipTexts =
        new Dictionary<PieceType, (string Name, string Description)>
        {
            ...
            },
            {
                ...
                (
                    "得分棋",
                    "每次受到碰撞/位移时得 2^n 分（n 为本次拍击内该棋被触发的次数），不同效果分别计数。"
                )
            },
            {
                PieceType.Explosion,
                (
                    "爆炸棋",
                    "被撞后向六邻各推 1 格，每推动/推出 1 枚 +2 分，被推棋子可继续碰撞。"
                )
            },
            {
                PieceType.Split,
                (
                    "分裂棋",
                    "被撞后消失，在撞击方向 ±60° 各生成 1 枚，挤推可碰撞，无法生成则就近空格放置。"
                )
            },
            {
                PieceType.Bounce,
            ...
            },
            {
                ...
                (
                    "吞噬棋",
                    "被撞后吞掉六邻所有棋子（含撞击者），每吞 1 枚 +3 分，被吞棋不触发能力，自身消失。"
                )
            },
            {
                PieceType.Turn,
                (
                    "转向棋",
                    "被撞后使撞击者顺时针转 60° 并前进 1 格，阻挡则停，成功后可继续碰撞。"
                )
            },
            {
                PieceType.Swap,
                (
                    "交换棋",
                    "被撞后与撞击者交换位置，撞击者继续沿原方向移动到最远并产生碰撞。"
                )
            },
            {
                PieceType.Whirlwind,
                (
                    "旋风棋",
                    "被撞后不动，周围非墙格顺时针轮换 1 格，移动棋子按新位置方向继续碰撞。"
                )
            },
        };
