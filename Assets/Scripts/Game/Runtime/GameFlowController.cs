...
using Ciga2026.Game.Config;
using Ciga2026.Game.Presentation;
using Ciga2026.Game.UI;
...
namespace Ciga2026.Game.Runtime
{
    public sealed class GameFlowController : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            if (hud != null)
            {
                if (hud.RotateLeftButton != null)
                {
                    hud.RotateLeftButton.onClick.AddListener(RotateCounterClockwise);
                }

                if (hud.RotateRightButton != null)
                {
                    hud.RotateRightButton.onClick.AddListener(RotateClockwise);
                }

                if (hud.SmackButton != null)
                {
                    hud.SmackButton.onClick.AddListener(Smack);
                }

                if (hud.SkipChoiceButton != null)
                {
                    hud.SkipChoiceButton.onClick.AddListener(SkipChoice);
                }
            }
        }
        ...
        private void OnDisable()
        {
            if (hud != null)
            {
                if (hud.RotateLeftButton != null)
                {
                    hud.RotateLeftButton.onClick.RemoveListener(RotateCounterClockwise);
                }

                if (hud.RotateRightButton != null)
                {
                    hud.RotateRightButton.onClick.RemoveListener(RotateClockwise);
                }

                if (hud.SmackButton != null)
                {
                    hud.SmackButton.onClick.RemoveListener(Smack);
                }

                if (hud.SkipChoiceButton != null)
                {
                    hud.SkipChoiceButton.onClick.RemoveListener(SkipChoice);
                }
            }
        }
        ...
        private void OnGUI()
        {
            ...
            GUILayout.BeginArea(new Rect(16f, 16f, 240f, 180f), GUI.skin.box);
            ...
            if (GUILayout.Button("A / Left"))
            {
                RotateCounterClockwise();
            }

            if (GUILayout.Button("D / Right"))
            {
                RotateClockwise();
            }
            ...
            if (GUILayout.Button("Space / Smack"))
            {
                Smack();
            }
        ...
        public void StartLevel(int index)
        {
            ...
            {
                hud?.ShowMessage("缺少 LevelConfig，先创建并拖到 GameFlowController。");
                return;
            }
            ...
            ClearViews();
            board.Clear();
            board.SetHexagonShape(level.boardRadius);
        ...
        public void RotateClockwise()
        {
            ...
            boardOrientation = Hex.WrapDirection(boardOrientation + 1);
            boardView?.SetOrientation(boardOrientation);
            RefreshHud();
        }
        ...
        {
            var target = levels[levelIndex].targetScore;
            EnterState(totalScore - target > 0 ? GameState.LevelSuccess : GameState.LevelFail);
        }
        ...
        {
            var level = levels[levelIndex];
            var chosen = WeightedPicker.Pick(level.choicePool, random);
        ...
        private bool TryPlaceRandomPiece(PieceType type)
        {
            ...
            var target = emptyCells[random.Next(0, emptyCells.Count)];
            return TryPlacePiece(type, target);
        }
        ...
        private bool TryPlacePiece(PieceType type, Hex target)
        {
            ...
            var piece = new Piece(type) { View = view };
            if (!board.PlacePiece(piece, target))
            {
                if (view != null && pieceViewFactory != null)
                {
                    pieceViewFactory.DestroyView(view);
                }
